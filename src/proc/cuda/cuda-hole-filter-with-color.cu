...
void librealsense::hole_filter_with_color_cuda_helper::hole_filter_process( uint16_t* h_depth_out, const uint16_t* h_depth_in, const float* h_lab_in, const int kernel_w, const int width, const int height )
{
	// 配列の要素数とバイト数
...
// P値の係数(空間距離)の初期化
void librealsense::hole_filter_with_color_cuda_helper::initializeCoef()
{
